...
public class Main extends JFrame{
	...
	public Main() {
		...
		JPanel Panel2 = new JPanel(new FlowLayout(FlowLayout.RIGHT,25,40));
...
//		getContentPane().setBackground(Color.WHITE);
		...
		iconjb1.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				if(!(jtf.getText().isEmpty())) {
					JOptionPane.showMessageDialog(null, "친구를 검색합니다.");
					connect();
					FriendVO friendVO = findFriendName(jtf.getText());
					if(friendVO!=null) {
					...
					JOptionPane.showMessageDialog(null, "검색할 친구이름을 입력해 주세요.");
				}
			}
		});
	...
	FriendVO findFriendName(String friendName){
		FriendVO vo = null;
		try {
			sql = "select *from kkakkao_friends where F_name = ?";
			pstmt = con.prepareStatement(sql);
			...
			if(rs.next()) {
				...
				vo = new FriendVO(F_id, F_name, F_gender, F_phone, F_email, F_Edate, F_photo);
			}
			rs.close(); pstmt.close(); con.close();
	...
	String finduser_name() {
		...
		try {
			sql = "select user_name from kkakkao_member where user_login = ?";
			pstmt = con.prepareStatement(sql);
			pstmt.setInt(1, 1);
			rs = pstmt.executeQuery();
			while(rs.next()) {

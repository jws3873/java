 ...
 * 예) TV,Computer, book,의자, 사람 등등
 * -실 세계의 객체들은 자신만의 고유한 특성 (속성)과 행동(기능)을 가지고 있음.
 * -이러한 객체들은 다른 객체들에게 행동을 요청하거나 정보를 주고 받는 등 
 * 상호 작용을 하면서 존재함.
 * 예를 든면 사람은 전자계산기의 기능을 이용하게 되고, 전자계산기는 
 * 계산결과를 사람에게 알려주는 상호작용을 하게 됨.
 ...
 * 이러한 관계의 종류에는집합관계, 사용관계, 상속관계가 있음. ==> 중요함.
 * -집합관계는 객체는 하나의 부품이고, 하나의 완성품에 해당이 됨.
 * 예를 든다면 자동차는 엔진, 타이어, 핸들, 등을 구성되므로 이 부품들을
 * 집합관계로 볼 수 있음.
 * -사용관계는 객체 간의 상호 작용을 말함. 다른 객체의 메서드를 호출하여 원하는 결과를
 * 얻어내는 것을 말함. 예를 든다면 사람은 자동차를 사용하므로 사람과 자동차는사용의
 * 관계라고 볼 수 있음.사람은 자동차를 사용할 때 달린다, 멈춘다 등의 메서드를 호출함.
 * -상송관계는 상위(부모) 객체를 기반으로 하위(자식) 객체를 생성하는 관계를 말함.
 * 일반적으로 상위객체는 종류를 의미하고, 하위객체는 구체적인 사물에 해당이 됨.
 * 예를 단드면 "독수리는 새의 종류이다" 에서 새(상위) 독수리(하위)는 상속관계에 
 * 있다고 볼 수 있음.
...
public class Ex01_Class {
	...
	// 알아서 해당 자료형의 default 값을 설정 을 해 줌
	int num; //멤버변수 - 전역변수
	...
	public static void main(String[] args) {
		...
		// 1단계 : 클래스 선언
		// 형식) 클래스이름 참조변수;
		//			*참조변수 : heap 메모리 공간에 객체가 만들어 지는데
		//					 만들어지는 객체의 주소값을 가지는 변수.
		

		// 2단계 클래스를 메모리(heap) 에 생성
		//형식) 참조변수 = new 클래스 이름(); ==> 클래스이름(x), 생성자(o)

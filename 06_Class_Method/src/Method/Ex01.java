 ...
 * 4. 자주 반복되는 내용을 정의해놓고, 필요할 때 호출하여 사용함.
 * 
 * 형식)
 * [접근제한] 반환형 메서드 이름(매개변수 혹은 인자){
 * 메서드 호출 시 실행될 문장;
 * }
 * -접근제한 : pubilc > protected> default > private
 * (클래스, 메서드, 변수 앞에 사용됨)
 ...
 * -만약 결과를 돌려줄 필요가 없는 경우에는 void 라는 키워드를 작성함.
 * -반환형이 void인 경우를 제외하고 결과를 되돌려주어야 할때는 메서드 
 * 명령문 맨 마지막에 return 이라는 키워드를 작성 후, 앞에서 선언한
 * 반환형과 같은 자료형으로 결과를 되돌려 준다.
 * 
 * -메서드 이름: 식별자, 소문자로 시작, 두개의 단어가 결합이 된 경우에는
 * 두 번째 글자의 첫 글자는 대문자로 작성하는 것이 관례이다.(CamelCase)
 * 

 ...
 * 6. 추상클래스는 추상 메서드를 가진 클래스를 말함.
 * 7. 물론 일반 멤버(멤버변수,멤버메서드)를 가질 수 있음.
 * 8.그러나 한개 이상의 추상메서드는 반드시 존재해야함.
 * 형식)
 * [접근제한] abstract class 클래스명{}
 * 9.용도: -응용프로그램에서 설계와 구현부분을 분리해서 작업 시 사용됨.
 * -추상클래스의 책의 목차에 비유를 하면, 서브클래스는 목차에 따라 
 * 완성된 책과 같음. 책을 쓸때도 목차를 잡아 놓고 책을 스면 훨씬
 * 쉽고 빠르며 방향이 흐트러지지 않는 것처럼 추상클래스를 이용하면
 * 응용 프로그램의 설계와 구현을 분리할 수 있음.
 * 추상클래스로 기본 방향을 잡아 놓고 서브클래스에서 구현하면 구현작업이 쉬워짐.
 * 
 */

/*
 * 추상클래스에서 error 발생 2가지
 * 1. 자식클래스에서 추상메서드를 재정의(o)
 * 2. 자식클래스에서 재정의를 하지 않는 경우.
 * ==> 자식 클래스 이름 앞에 abstract 키워드를 붙여야 함.
 */

 public abstract class Super { //추상클래스
	 int num1;	// 인스턴스 멤버변수
	 int calc() {	// 인스턴스 멤버 메서드
		 int sum = num1 +100;
		 return sum;
	 }

 ...
 * -이들 접근지정자 중 protected, private 접근지정자는 클래스 앞에는 사용을 못함.
 * 단, Inner class(중첩클래스) 앞에는 사용이 가능함.
 * 
 * 1. private: - 동일한 클래스에서만 접근이 가능함.
 * -외부에서 인스턴스 변수를 바로 접근이 가능하게 제어를 하면
 * 잘못된 데이터를 저장할 수 있는 문제가 발생함.
 * -따라서 직접 접근을 제어하고 메서드(setter() / getter())를 사용하여 접근하게 함.
 * 
 * 2. default : - 동일한 패키지에서만 접근이 간능함. default 키워드는 생략가능.
 * -같은 클래스 멤버, 같은 패키지 안의 다른 클래스의 멤버까지만 접근이 가능함.
 * -다른 패키지의 상속 관계에 있는 클래스라도 접근 불가.
 * 
 * 3. protected : 같은 클래스의 멤버, 같은 패키지 안의 다른 클래스의 멤버,
 * 다른 패키지의 상속관계에 있는 멤버까지 접근이 가능함.
 * 
 * 4. public : -모든 클래스에서 접근이 가능함.
 * -같은 클래스의 멤버, 같은 패키지 안의 다른 클래스의 멤버,
 * 다른 패키지의 상속관께에 있는 멤버, 다른 패키지의 클래스의 
 * 멤버까지 접근이 가능함.
 * 
 * -접근범위 (scope) : public > protected > default > private
 * 
 * 
 */
public class Super {
